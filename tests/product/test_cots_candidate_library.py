...
class CotsCandidateLibraryTests(unittest.TestCase):
    ...
    def test_exact_catalog_candidate_is_not_bom_exact_match(self):
        ...
        self.assertEqual(catalog["part_number"], "23LC1024-I/SN")
        self.assertEqual(catalog["identity_status"], "PARTIAL_UNRESOLVED")
        self.assertIn("EXACT_TEST_ARTICLE_IDENTITY_UNRESOLVED", [b["stable_code"] for b in catalog["blockers"]])

    def test_catalog_and_family_level_flight_experiment_are_kept_separate(self):
        records = {r["source_identity"]: r for r in run_node("resolve", self.sample)["records"]}
        flight = records["ESA GOMX-4B CHIMERA Board"]
        coverage = {e["event_type"]: e["status"] for e in flight["coverage"]}
        self.assertEqual(flight["part_number"], "23LC1024")
        self.assertEqual(coverage["SEU"], "DOCUMENT_NOT_SELECTED")
        self.assertEqual(coverage["SEL"], "DOCUMENT_NOT_SELECTED")
        self.assertEqual(coverage["TID"], "EVIDENCE_MISSING")
    ...
    def test_family_only_substitution_fails_closed(self):
        ...
        attack["records"][0]["part_number"] = "23LC1024-family"
        self.assert_closed(run_node("resolve", attack), "OPTIMISTIC_RECORD_REJECTED")

    def test_seu_cannot_substitute_for_sel_or_other_destructive_see(self):
        for target in ("SEL", "SEB", "SEGR"):
            attack = copy.deepcopy(self.sample)
            event = next(e for e in attack["records"][1]["coverage"] if e["event_type"] == target)
    ...
    def test_allowlist_export_has_no_pdf_locator_numbers_or_decision_claim(self):
        ...
        for forbidden in ["record_id", "manufacturer", "part_number", "source_identity", "next_action_ko", "http", ".pdf", "lot", "date_code", "approver", "suitability", "23lc1024", "chimera", "ds20005142c"]:
