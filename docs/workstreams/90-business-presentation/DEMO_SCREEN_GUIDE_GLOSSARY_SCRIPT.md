...
- 부품 identity에는 manufacturer, exact part number(PN), process, die와 lot가 필요하다. 같은 제품군 이름이라도 제조 공정·die·lot가 달라지면 시험 근거의 적용성이 달라질 수 있다. 구매 수량은 exact-part 승인과 차폐 판단에서 제외하며, 여러 동일 장치의 총 SEU율을 계산할 때만 별도 분석 범위 입력을 사용한다.
- Core 화면의 LEO 550 km, 97.6°, 1년과 `Example Semi / EX-100 / P1 / LOT-A`는 모두 합성 fixture다. 발표의 catalog-level 검토 대상 `Microchip 23LC1024-I/SN`과 섞지 않는다.
- COTS catalog 검토 대상은 1건이지만 exact 비행·시험 article과 decision-usable evidence packet은 0건이므로 실제 추천이나 구매 근거로 쓰지 않는다.

**발표자가 전달할 한 문장**

> 임무 조건과 제조사·정확한 부품번호·공정·die·lot을 함께 고정해야 시험 근거의 적용성을 추적할 수 있습니다.

**20~35초 대본 초안 — `PLANNED 23초`**

“여기서는 합성 LEO 임무와 메모리 부품 하나로 질문을 고정합니다. BOM은 부품 목록이고, 이름만 맞추는 것이 아니라 제조사, 정확한 부품번호, 공정, die, lot까지 확인해야 합니다. 발표의 COTS 검토 대상은 Microchip 23LC1024-I/SN이지만, 비행 exact suffix와 시험 근거가 아직 연결되지 않아 판단은 HOLD입니다.”
...
| 2 | 23초 | “그래서 합성 임무 하나와 부품 하나에서 시작합니다. BOM은 부품 목록이며 제조사, 정확한 부품번호, 공정, die, lot을 함께 확인해야 합니다. 발표에서는 현재 양산 COTS SRAM 23LC1024-I/SN을 검토 대상으로 고정했지만 비행 exact identity와 시험 근거가 아직 없어 HOLD합니다.” |
