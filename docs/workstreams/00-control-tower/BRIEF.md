...
- 검증 실패: 해당 `CURRENT.md`를 `CHANGES_REQUESTED`로 갱신하고 commit·push를 보류하며, 현재 작업 채팅에 전달할 change request Markdown을 해당 Workstream의 ignored `instructions/`에 생성하거나 갱신해 사용자에게 링크로 전달한다. 작업 채팅이 완료 후 작성하는 handoff는 같은 계층의 `handoffs/`에 둔다.
...
- 같은 Workstream의 새 채팅은 다음 1단위 번호로, 다음 Workstream의 첫 채팅은 해당 십 단위 번호로 시작한다. 새 채팅이 실제로 필요할 때만 해당 Workstream의 ignored `instructions/`에 시작 템플릿·작업 지침·change request를 생성해 사용자에게 링크로 전달한다. 작업 완료 handoff는 같은 계층의 ignored `handoffs/`에 두며 프로젝트 commit에 포함하지 않는다.
...
- 모든 작업 지침과 change request에는 `병렬 가능 작업`을 포함한다. 후보가 있으면 채팅 번호, 소유 파일, 의존성, Exit Gate를 적고, 없으면 `없음`과 이유를 적는다.
