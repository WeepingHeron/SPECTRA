...
- 발표·검증 콘솔은 별도 공개 Cloud Run `spectra-demo-console` revision `00013-8vp`에 배포됐다. `문서 검사`는 요청마다 `pypdf/TXT` 규칙 기반 parser를 실행하고 기본 정보 확인과 사건별 요구자료를 분리한다. `임무·부품·시험 연결`은 고정 합성 Mission Case와 사용자가 역할을 지정한 세 문서의 후보 교차 대조를 지원하며, 결과는 `근거 연결 · TID·SEU 계산 · 최종 적용성 관문`으로 나눠 보여준다. 직접 문서 경로는 승인·권리·Mission Case 결속 전까지 `NOT_FOR_DECISION / HOLD`이며, 결과는 원문을 제외한 결정론적 검토 패킷 JSON으로 내려받을 수 있다. `저장된 공격 검증`은 공개 쓰기 권한을 열지 않고 독립 확인된 snapshot을 표시하며, `문서별 결과표`는 공개 GCP catalog를 읽는다.
- 최신 통합 회귀는 unit 449개와 Assurance 공격 실행 47개가 통과했고, 공개 1280×720 deck·Console 동선을 확인했다. 기존 7분본은 구두 리허설했으며 사람 10분 낭독·탭 전환 시간은 아직 `NOT_MEASURED`다.
