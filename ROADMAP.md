...
| 5 | 완화·사용자 정책 엔진 | 50 Mitigation & Policy | `50` | 20 Simulation, 40 Parts, 60 Assurance | 환경·부품 증거 인터페이스 | **IN_PROGRESS** — H02 계약·v2 schema, 합성 Decision Engine H02와 WATCHDOG·TMR·SEL runtime calculator H03 검증; 실제 효과·실제 evidence 연결 미구현 |
| 6 | 독립 보증·평가 기준선 | 60 Assurance & Evals | `60` | 20~50 구현 Workstream | 결정론적 계산·증거·정책 경로 | **IN_PROGRESS** — H03 runtime calculator 포함 평가된 공격 실행 47개 False PASS 0 검증; 실제 GCP D02는 `NOT_EVALUATED` |
| 7 | Multi-Agent·GCP 실행 경로 | 70 Platform & GCP | `70` | 30~60 전문 Workstream | Stage 3~6 서비스·감사 계약 | **IN_PROGRESS** — 저장·권리 gate와 local raw-manifest preflight H03 통합; 실제 resource·rights·manifest·실행·비용 증거는 0 |
| 8 | 제품·대시보드 통합 | 80 Product & Dashboard | `80` | 60 Assurance, 70 Platform | 검증된 통합 API와 EvidencePacket | `IN_PROGRESS` — H09 runtime integrity 검증; 실제 API·원문 evidence·Stage 7 resource 미통합 |
| 9 | 비즈니스·발표·최종 시연 | 90 Business & Presentation | `90` | 60 Assurance, 80 Product | Stage 8 제품 기준선 | `IN_PROGRESS` — 7분 발표·4분 Product UI·3분 Q&A 원고 통합; 사람 리허설과 사용자·비즈니스 검증은 미완료 |
...
현재 상태는 `MVP IN_PROGRESS`다. 합성 계산·결정 엔진·runtime 완화 계산·공격 기준선과 build-time Product 결과 연결은 존재하지만 실제 환경·부품 근거 경로, runtime 결과 UI 연결과 실제 API 연결이 남아 있다.
